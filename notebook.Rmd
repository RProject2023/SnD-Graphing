...
linear_curve_2_points(p1, title = "Supply", color = "red", pointsHidden = FALSE, lineCoordinateHidden = FALSE)
```
...
```{r}
p2 <- c(2, 9, 5, 5)
linear_curve_2_points(p2, title = "Demand", color = "blue", pointsHidden = FALSE, lineCoordinateHidden = FALSE)
```

```{r}
linear_curve_2_points(p1, p2, title = "Supply & Demand", color = c("red", "blue"))
```
...
curve_N_points(supply, demand, title = "Supply & Demand", color = c("blue", "red"))
```
...
gg <- curve_N_points(supply, demand, demand2, title = "Supply & Demand", color = c("blue", "red", "pink"))
...
```{r}
supply_sr <- bezier(x = c(1, 8, 9), y = c(1, 5, 9)) %>% as.data.frame()
demand_sr <- bezier(x = c(1, 3, 9), y = c(9, 3, 1)) %>% as.data.frame()

gg_sr <- curve_N_points(supply_sr, demand_sr, title = "Supply & Demand", color = c("blue", "red"))
...
```{r}
supply_srp <- bezier(x = c(1, 8, 9), y = c(1, 5, 9)) %>% as.data.frame()
demand_srp <- bezier(x = c(1, 3, 9), y = c(9, 3, 1)) %>% as.data.frame()

gg_srp <- curve_N_points(supply_srp, demand_srp, title = "Supply & Demand", color = c("blue", "red"))
...
-   **curve**: krivulja potrošnje

-   **x**: X vrijednosti gdje se nalaze sjecišta
...
```{r}
curve <- bezier(x = c(1, 8, 9), y = c(1, 5, 9)) %>% as.data.frame()

indif <- indifference_curve(curve, x = c(2, 5, 8), labels = c("A", "B", "C"))
...
production_possibility_frontier(curve_data1, curve_data2, colors = c("red", "blue"), x = c(3, 7, 10), labels = c("A", "B", "C", "D", "E", "F"))
```
...
move_curves_along_x_axis(curve_data, offset = 5)
move_curves_along_x_axis(curve_data, offset = 20, is_percentage = TRUE)
```